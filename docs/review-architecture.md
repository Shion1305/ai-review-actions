...
一方、必要な結果や途中の判断まで早く消すと、同じファイルの閲覧や確認を繰り返す原因になる。
保持する会話は固定の往復数ではなくバイト数で判断し、入力予算に収まる調査履歴を残す。
...
モデルへ送る会話と観測の保管場所を分離する。会話を直列化したバイト数の目標は
64,000バイトと`model-input-byte-limit`の3分の2の小さい方とする。
最初の依頼・作業メモ・調査索引を含めて目標内なら、過去の会話を保持する。
各ツール出力は約6KBまで残し、古くなったという理由だけでは短縮しない。

目標を超える場合に限り、古い会話からモデルの応答とそれに対応するツール結果をまとめて外し、
最大6KBの索引へ所在情報を残す。最初の依頼と最新の会話は維持する。
ツール呼び出しと応答の組、必要なプロバイダー署名を保ち、未完了の呼び出しや遅れて返る結果を
分断しない。そのため会話用の目標だけでは厳密な上限を保証せず、要求全体のバイト制限を別途適用する。
...
PR説明は`get_review_context(section="description", body_start=...)`で1,000文字ずつ読み、
`next_body_start`を次の開始位置にする。スレッドやコメントの項目番号は`start_index`で指定し、
長いコメントは同じ項目番号のまま本文の開始位置だけを進める。次の項目へ移るときは本文の
開始位置を0へ戻す。本文の続きと一覧の続きを別のカーソルで扱い、同じページの再取得を避ける。
...
履歴・ツール定義・指示・設定を直列化して、既定で1要求96,000バイトまでに制限する。
出力は8,192トークンまで。これらの入力バイト数は
プロバイダーのトークン数や実際のHTTPボディサイズと同一ではない。
成功した応答の実トークン使用量もログに出し、運用上の検証と調整に使う。
応答が選択したツール名もJSONで記録する。引数や出力本文を追加で載せず、改行などをエスケープして
1行の記録に保つ。実行済みかどうかは観測ログと合わせて確認する。
サンドボックスの操作は開始時にもコマンドと制限時間をJSONで記録する。
結果が返らない操作も特定できるようにし、完了の記録とは区別する。

要求ごとの入力上限を満たす場合は次の要求へ進み、分間送信量や固定の要求間隔による
先回りの待機は行わない。429・502・503が返された場合は最大3試行まで再送する。
`Retry-After`とGoogleの`RetryInfo`に従い、指定がない場合はバックオフして再試行する。
再試行待機の合計は1要求180秒までに制限する。
SDK独自の再試行を止め、再送によりツールの副作用が再実行されることを防ぐ。
429が続く場合や要求ごとの入力上限・再試行待機の上限を超えた場合は、承認できない未完了レポートへ変換する。

GeminiのTPMは入力トークンの制限であり、RPM等と別に評価され、プロジェクト全体で共有される。
プロジェクト全体の送信量は本Actionでは制御しないため、別PRや別アプリも同じプロジェクトを使う場合は
合計使用量を確認する。[Googleのレート制限](https://ai.google.dev/gemini-api/docs/rate-limits)。
...
単体テストでは入力バイト制限・予算内の履歴保持と超過時の削減・待機時間・429回復と打ち切り・
再試行時のツール非再実行、実Gitの差分とPR説明のページ化、議論だけでは完了できないことを検証する。
