...
`get_review_context`のPR説明は1回に1,000文字ずつ読み、`next_body_start`の値を
次の呼び出しの`body_start`に指定します。説明の取得では`start_index`を0のままにします。
スレッド一覧やコメント一覧では`next_index`で項目を進め、長いコメントの本文は同じ
`start_index`のまま`body_start`を進めます。本文を読み終えて次のコメントへ移るときは
`body_start`を0に戻します。
...
モデル応答のログには選択したツール名をJSON形式で記録します。観測IDと照合し、調査の進み方や
同じ操作の繰り返しを確認できます。ツール名の記録だけで、その操作が実行済みとは判断しません。
サンドボックスの操作は開始時にも`AI tool start:`としてコマンドと制限時間を記録するため、
結果がまだ返らない場合でも、どの操作を待っているか確認できます。
...
モデルへ渡す各ツール出力は約6KBに制限し、会話の保持は直列化したバイト数で判断します。
会話用の目標は64,000バイトと`model-input-byte-limit`の3分の2の小さい方です。
目標内では過去の会話とツール出力を保持し、古いという理由だけで出力を短縮しません。
超過時は古い会話からツール呼び出しと応答の組を外し、最初の依頼、最新の会話、作業メモ、
最大6KBの調査索引を残します。観測自体は別のメモリに保持するため、`read_observation`で
必要な結果をページ単位で取得できます。
未完了のツール呼び出しとの対応を保つため、会話が目標を超えて残る場合があります。
指示・ツール定義・設定を含む要求全体には、表の入力上限を別途適用します。
短い作業メモには結論・指摘候補・未確認の疑問を残し、毎回置き換えます。メモも根拠IDと
サイズを検証し、確定した証拠としては扱いません。
巨大な差分は全件投入せず、ファイル一覧から必要な差分を最大400行ずつ読みます。

要求ごとの入力上限を満たす場合は、ツールの結果を受け取り次の要求へ進みます。
内部の分間送信量予算や固定の要求間隔は設けません。旧入力`model-input-bytes-per-minute`は削除したため、
利用側ワークフローに指定している場合は取り除いてください。
429・502・503が返された場合は、最大3試行まで待機して再送します。`Retry-After`とGoogleの
`RetryInfo`を尊重し、SDK内部の重複リトライは無効化します。再送ではツールを再実行しません。
再試行の待機は1要求で合計180秒までです。要求ごとの入力上限・再試行待機の上限を超えた場合や
429の継続時は未完了レポートを返し、承認しません。認証エラー等は通常の失敗として扱います。

Geminiの制限はAPIキー単位ではなくプロジェクト単位なので、
...
短いページ読み取りと大量出力のコマンドを回数だけで等価に扱わず、要求ごとの入力サイズと
